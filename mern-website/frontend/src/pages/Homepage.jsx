        ...
        navigate("/analysis", {
          state: {
            ...
            messages: data.messages || [
              ...
              { role: "user", content: data.speech_to_text, user_audio: audioUrl },
              { role: "assistant", content: data.doctor_response, assistant_audio: data.doctor_voice_url },
            ],
          },
        });
      } catch (err) {
        setError("Analysis failed. Please check if your server is running and try again.");
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <p className="text-gray-400 text-lg max-w-2xl mx-auto">
                Upload an image of your symptoms and describe them with your voice. Our AI will analyze and provide recommendations.
              </p>
            </div>
                ...
                <div className="relative border-2 border-dashed border-gray-700 rounded-lg p-6 text-center hover:border-blue-500 transition-colors min-h-[200px] flex items-center justify-center">
                  {imagePreview ? (
                    <div className="space-y-4 w-full">
                      <img src={imagePreview} alt="Preview" className="max-w-full max-h-64 mx-auto rounded-lg shadow-xl" />
                      <button onClick={removeImage} className="text-red-400 hover:text-red-300 text-sm underline">Remove image</button>
                    </div>
                  ) : (
                  ...
                  <div className="flex flex-col items-center justify-center min-h-[200px]">
                    {!isRecording && !audioUrl && (
                      <div className="flex flex-col items-center justify-center h-full w-full">
                        <button onClick={startRecording} className="flex items-center justify-center w-24 h-24 rounded-full bg-green-600 hover:bg-green-700 hover:scale-105 transition-all duration-300 shadow-lg">
                          <Mic className="h-12 w-12 text-white" />
                        </button>
                        <p className="text-gray-400 mt-4 text-sm">Click the microphone to start recording</p>
                      </div>
                    )}
                    ...
                    {isRecording && (
                      <div className="flex flex-col items-center justify-center w-full">
                        ...
                        <div className="flex items-center justify-center gap-4">
                          <button onClick={stopRecording} className="flex items-center justify-center w-16 h-16 rounded-full bg-red-600 hover:bg-red-700 transition-all duration-300 shadow-lg">
                            <MicOff className="h-6 w-6 text-white" />
                          </button>

                          <button onClick={togglePause} className="flex items-center justify-center w-16 h-16 rounded-full bg-yellow-500 hover:bg-yellow-600 text-black transition-all duration-300 shadow-lg">
                            {isPaused ? "▶" : "⏸"}
                          </button>
                        </div>
                      </div>
          ...
          <div ref={waveformContainerRef} className="w-full h-14" />
                  ...
                  {isRecording && (
                    <div className="text-center">
                      ...
                      <p className="text-gray-400 text-sm">{isPaused ? "Recording paused" : "Recording..."}</p>
                    </div>
                  )}
                </div>
